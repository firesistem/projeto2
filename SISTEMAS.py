    ...
    return {
        ...
        # CONCENTRAÇÃO DE PÚBLICO
         ...
         "Museus e bibliotecas": {
            "glp": True,  # Adicionado "glp" em qualquer situação, conforme NT 01/16
            "nivel1": {
                "condicoes": {"area": 12.01, "altura": 9.01},
        ...
        # Grupo 22
         "Artes cênicas e auditórios": {
            ...
            "nivel5": {
                "condicoes": {"area": 3000.01, "altura": 3.01},
 ...
 # Grupo 23
      ...
      # Grupo 25
         "Restaurantes": {
            "glp": True,  # Adicionado "glp" em qualquer situação, conforme NT 01/16
            "nivel1": {
                "condicoes": {"area": 750.01, "altura": 6.01},
                "sistemas": ["Hidrante", "Alarme"]
            },
            "nivel2": {
                "condicoes": {"area": 5000.01, "altura": 12.01},
...
if tipo_selecionado and ocupacao_selecionada and area > 0 and altura > 0:
    ...
    #  Define a variável regras com um valor padrão (None)
    regras = None
    ...
    if ocupacao_sem_grupo in regras_ocupacao:
        ...
        if regras.get("glp"):  # Verifica si a chave "glp" existe e é True
           ...
           # Lista para rastrear os sistemas já exibidos
        sistemas_exibidos = []

        # ADICIONADO: Lógica para Garagens com Subsolo
        possui_subsolo = "Não"  # Valor padrão
        numero_subsolos = None

        if ocupacao_sem_grupo == "Garagens em geral":
            possui_subsolo = st.selectbox("A edificação possui subsolos?", ["Não", "Sim"])

            if possui_subsolo == "Sim":
                numero_subsolos = st.selectbox("Escolha o número de subsolos", ["1", "2", "igual ou superior a 3 subsolos"])

        # Verificar primeiro nível de exigência
        if regras and "nivel1" in regras:
        ...
        # ADICIONADO: Condição para Hidrante em Garagens com Subsolo (altura > 3m)
        if ocupacao_sem_grupo == "Garagens em geral" and possui_subsolo == "Sim" and altura > 3.00:
            if "Hidrante" not in sistemas_exibidos:
                st.write("Hidrante")
                sistemas_exibidos.append("Hidrante")
        ...
        # ADICIONADO: Condição para Detecção e Chuveiros em Garagens com 3+ Subsolos
        if ocupacao_sem_grupo == "Garagens em geral" and possui_subsolo == "Sim" and numero_subsolos == "igual ou superior a 3 subsolos":
            sistemas_adicionais = ["Detecção de incêndio", "Chuveiros automáticos"]
            for sistema in sistemas_adicionais:
                if sistema not in sistemas_exibidos:
                    st.write(sistema)
                    sistemas_exibidos.append(sistema)
